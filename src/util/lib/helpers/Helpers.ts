	...
	public async antispamRepeatingMessages(message: Message): Promise<void>
	{
		...
		if (message.member.spamCounter === 4) {
			if (antispamRepeatingMessagesEnabled) {
				message.channel.send(`<@${message.member.id}>, You are sending too many messages too quickly, or the same message too many times. Please wait **at least 5 minutes** before sending another message or you will be muted.`);
				message.delete()
					.then((msg) => { return; })
